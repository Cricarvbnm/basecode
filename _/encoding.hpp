...
Encoding<EncodingID::base16>::getInitialSet() noexcept {
    ...
    int i = 0;
    for (char chr = '0'; chr <= '9'; ++chr, ++i) result_set[i] = chr;
    for (char chr = 'A'; chr <= 'F'; ++chr, ++i) result_set[i] = chr;
...
Encoding<EncodingID::base32>::getInitialSet() noexcept {
    ...
    int i = 0;
    for (char chr = 'A'; chr <= 'Z'; ++chr, ++i) result_set[i] = chr;
    for (char chr = '2'; chr <= '7'; ++chr, ++i) result_set[i] = chr;
